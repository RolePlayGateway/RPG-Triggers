...
switch (true) {
	...
	*/
	...
	case preg_match('/I((\'?m)|( am))(.*)bored/i',$text):
		$this->insertChatBotMessage($this->getPrivateMessageID(),"Boredom is a sign of one's inability to be imaginative!  Go [url=http://www.roleplaygateway.com/roleplay/]browse open roleplays[/url] or [url=http://www.roleplaygateway.com/ucp.php?i=roleplays&mode=new]create a new one[/url].");
	break;
...
/* 	case stristr($text,"!barrelroll"):
	...
	*/
	...
	case stristr($text,"!google"):
		...
		$searchText = preg_replace("/\[\/color]/","",$searchText);
